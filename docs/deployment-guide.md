...
1.  **Ekspor Statis**: Mengaktifkan `output: 'export'` dan menonaktifkan optimasi gambar bawaan server (`unoptimized: true`) pada file [next.config.mjs](file:///home/amirfaisalz/Documents/hamzah/mewdon_/next.config.mjs).
2.  **GitHub Actions**: Membuat file alur kerja otomatis (workflow) di [.github/workflows/deploy.yml](file:///home/amirfaisalz/Documents/hamzah/mewdon_/.github/workflows/deploy.yml).
...
3. Cari domain yang ingin Anda gunakan (misal: `mewdon.com`), lalu klik tombol **Kelola Domain**.
4. Masuk ke tab **DNS Management** atau **Zone Editor**.

### Langkah 2: Tambahkan Record DNS
Anda perlu menambahkan **4 Record A** (untuk domain utama/Apex domain seperti `mewdon.com`) dan **1 Record CNAME** (untuk subdomain `www.mewdon.com`).
...
1. Buka halaman repositori Anda di GitHub: `https://github.com/farhanhamzahhhh/mewdon`.
...
2. Di bagian **Custom domain**, masukkan nama domain kustom Anda (misal: `mewdon.com` atau `www.mewdon.com`).
3. Klik tombol **Save**.
...
*   File [CNAME](file:///home/amirfaisalz/Documents/hamzah/mewdon_/CNAME) di folder utama repositori Anda memastikan bahwa GitHub Pages mengetahui domain kustom mana yang terikat dengan proyek ini.
